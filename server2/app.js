...
const io = require("socket.io").listen(8002);
...
let availableServers = [0,0,1];
let formData = {};
let selectedValidator = -1;
let tsuccess;

let server0HasBeenAlive = false;
let server1HasBeenAlive = false;
...
const socket = io_client.connect('http://127.0.0.1:8001');

socket.on('connect', function () {
    socket.emit('server2Alive',1);
    console.log('server 2 connected to server 1');

    if (server1HasBeenAlive) {
        socket.emit('updateYourBlockchain', {blockchain:server2Chain.chain});
        console.log('Updated Blockchain sent to Server 1');
        socket.emit('updateYourServerWealth', {wealtharray:serverWealth});
        console.log('Updated Wealth array sent to Server 1');
    }
});

socket.on('disconnect', function(){
    //socket.close();
    console.log('server 2 disconnected from server 1');
    availableServers[1] = 0;
    console.log(availableServers);
    server1HasBeenAlive = true;
...
const osocket = io_client.connect('http://127.0.0.1:8000');

osocket.on('connect', function () {
    osocket.emit('server2Alive',1);
    console.log('server 2 connected to server 0');


    if (server0HasBeenAlive) {
        osocket.emit('updateYourBlockchain', {blockchain:server2Chain.chain});
...
osocket.on('disconnect', function(){
    //osocket.close();
    console.log('server 2 disconnected from server 0');
...
ioBrowser.on('connection', function(socket) {
    console.log('Client opened browser window on Server 2');
});



io.on('connection', function(socket){
    console.log('Another server has connected to Server 2');


    socket.on('server1Alive', function(data){
        availableServers[1] = data;
    ...
    socket.on('updateYourBlockchain', function(data){
        server2Chain.chain = [server2Chain.chain[0],server2Chain.chain[1],server2Chain.chain[2]];
        for (let n = 3; n<data.blockchain.length; n++){
            server2Chain.chain.push(data.blockchain[n]);
        }
        console.log('Server 2 blockchain is up to date');
        console.log(JSON.stringify(server2Chain, null, 4));
    ...
    socket.on('updateYourServerWealth', function(data){
        ...
        console.log('Server 2 wealth array is up to date');
...
let server2Chain = new BlockChain();
server2Chain.addBlock(new Block(Date.now(), {
...
function balanceOfClient(from) {
    let balance = 0;
    server2Chain.chain.forEach(function(block) {
...
function requestStatus(balance) {
    if (balance < formData.amount){
        ...
        serverWealth[2] += 3;
        console.log('Transaction Accepted by Server 2');
        console.log(JSON.stringify(server2Chain, null, 4));
        ioBrowser.emit('formStatus', 'Transaction Accepted');
        let sobj = {server:2, newblock: server2Chain.chain[server2Chain.chain.length - 1]};
        console.log('Object(Blockchain block) size in bytes: ' + jsonSize(sobj).toString());

        socket.emit('transactionAccepted', {server:2, newblock: server2Chain.chain[server2Chain.chain.length - 1]});
        osocket.emit('transactionAccepted', {server:2, newblock: server2Chain.chain[server2Chain.chain.length - 1]});
    }
