...
socket.on('connect', function () {
    socket.emit('server0Alive',1);
    console.log('server 0 connected to server 1');
    if (server1HasBeenAlive) {
        socket.emit('updateYourBlockchain', {blockchain:server0Chain.chain});
...
socket.on('disconnect', function(){
    //socket.close();
    console.log('server 0 disconnected from server 1');
...
osocket.on('connect', function () {
    osocket.emit('server0Alive',1);
    console.log('server 0 connected to server 2');
    if (server2HasBeenAlive) {
        osocket.emit('updateYourBlockchain', {blockchain:server0Chain.chain});
...
osocket.on('disconnect', function(){
    //osocket.close();
    console.log('server 0 disconnected from server 2');
...
ioBrowser.on('connection', function(socket) {
    console.log('Client opened browser window on Server 0');
});


io.on('connection', function(socket){
    console.log('Another server has connected to Server 0');
    ...
    socket.on('updateYourBlockchain', function(data){
        server0Chain.chain = [server0Chain.chain[0],server0Chain.chain[1],server0Chain.chain[2]];
        for (let n = 3; n<data.blockchain.length; n++){
            server0Chain.chain.push(data.blockchain[n]);
        }
        console.log('Server 0 blockchain is up to date');
        console.log(JSON.stringify(server0Chain, null, 4));
    ...
    socket.on('updateYourServerWealth', function(data){
        ...
        console.log('Server 0 wealth array is up to date');
...
let server0Chain = new BlockChain();
server0Chain.addBlock(new Block(Date.now(), {
...
function balanceOfClient(from) {
    let balance = 0;
    server0Chain.chain.forEach(function(block) {
...
function requestStatus(balance) {
  if (balance < formData.amount){
      console.log('Transaction Rejected by Server 0');
      //tsuccess = false;
      ...
      serverWealth[0] += 3;
      console.log('Transaction Accepted by Server 0');
      //tsuccess = true;
      console.log(JSON.stringify(server0Chain, null, 4));
      ioBrowser.emit('formStatus', 'Transaction Accepted');
      let sobj = {server:0, newblock: server0Chain.chain[server0Chain.chain.length - 1]};
      console.log('Object(Blockchain block) size in bytes: ' + jsonSize(sobj).toString());
      socket.emit('transactionAccepted', {server:0, newblock: server0Chain.chain[server0Chain.chain.length - 1]});
      osocket.emit('transactionAccepted', {server:0, newblock: server0Chain.chain[server0Chain.chain.length - 1]});
